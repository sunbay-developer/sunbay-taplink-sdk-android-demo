...
object Constants {
    ...
    // App-to-App mode uses full names; Cloud API uses single-letter codes
    // Reference: https://docs.sunbay.dev/zh/resources/reference/transaction-status
    object TransactionStatus {
        ...
        // Cloud API single-letter status codes
        const val CLOUD_INITIAL = "I"                // Created, not yet processing (non-terminal)
        const val CLOUD_PROCESSING = "P"             // Processing (non-terminal)
        const val CLOUD_SUCCESS = "S"                // Success (terminal)
        const val CLOUD_FAILED = "F"                 // Failed (terminal)
        const val CLOUD_CLOSED = "C"                 // Closed (terminal)
        
        /** Check if a Cloud API status code is terminal (no further changes) */
        fun isTerminal(status: String?): Boolean =
            status == CLOUD_SUCCESS || status == CLOUD_FAILED || status == CLOUD_CLOSED
    }
    
    // Identifier prefixes for transaction tracking and debugging
    const val ORDER_ID_PREFIX = "ORDER"
