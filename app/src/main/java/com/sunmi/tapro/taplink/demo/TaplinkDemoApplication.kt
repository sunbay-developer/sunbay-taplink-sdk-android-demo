...
class TaplinkDemoApplication : Application() {
    ...
    companion object {
        ...
        // Static context reference for accessing application context globally
    ...
    override fun onCreate() {
        ...
        // Sync payment service with saved print receipt preference
        TaplinkPaymentService.getInstance().initialize(this)
        TaplinkPaymentService.getInstance().setPrintReceipt(
            PrintReceiptMapping.toSdk(ConnectionPreferences.getPrintReceipt(this))
        )
    }
    ...
    private fun initializeTaplinkSDK() {
        try {
            ...
            // Read sensitive config from unified local secrets file via BuildConfig.
            val appId = BuildConfig.APP_TAPLINK_APP_ID
            val merchantId = BuildConfig.APP_TAPLINK_MERCHANT_ID
            val secretKey = BuildConfig.APP_TAPLINK_SECRET_KEY
            ...
            val maskedSecretKey = if (secretKey.length >= 8) {
                "${secretKey.take(4)}****${secretKey.takeLast(4)}"
            } else {
                "****"
            }
            Log.d(TAG, "Secret Key: $maskedSecretKey")
