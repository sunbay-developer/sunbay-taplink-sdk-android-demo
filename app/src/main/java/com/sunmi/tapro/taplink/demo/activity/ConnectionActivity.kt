...
class ConnectionActivity : AppCompatActivity() {
    ...
    private fun initViews() {
        ...
        // Setup print receipt spinner
        setupPrintReceiptSpinner()
    ...
    private fun loadLanConfig() {
        ...
        // Always set text explicitly to prevent hint from showing
        lanIpInput.setText(ip ?: "")
...
//        switchTls.isChecked = false // LAN mode defaults to TLS disabled
     ...
     * Setup print receipt spinner
     */
    private fun setupPrintReceiptSpinner() {
        // Create print receipt options
        val printReceiptOptions = arrayOf(
            "NONE (No receipt)",
        ...
        // Create adapter for spinner
        val adapter = ArrayAdapter(this, android.R.layout.simple_spinner_item, printReceiptOptions)
        adapter.setDropDownViewResource(android.R.layout.simple_spinner_dropdown_item)
        
        // Set adapter to spinner
        spinnerPrintReceipt.adapter = adapter
        
        // Load saved print receipt option or set default to NONE
        val savedPrintReceipt = ConnectionPreferences.getPrintReceipt(this)
        val selectedIndex = when (savedPrintReceipt) {
            "NONE" -> 0
            "MERCHANT" -> 1
            "CUSTOMER" -> 2
            "BOTH" -> 3
        ...
        spinnerPrintReceipt.setSelection(selectedIndex)
        ...
        Log.d(TAG, "Print receipt spinner setup with saved option: $savedPrintReceipt")
    }
...
//                }
    ...
    private fun saveConfig() {
        ...
        val selectedPrintReceiptIndex = spinnerPrintReceipt.selectedItemPosition
        val printReceiptValue = when (selectedPrintReceiptIndex) {
            0 -> "NONE"
            1 -> "MERCHANT"
            2 -> "CUSTOMER"
            3 -> "BOTH"
        ...
        ConnectionPreferences.savePrintReceipt(this, printReceiptValue)
        // Sync to payment service (from :service module)
        TaplinkPaymentService.getInstance().setPrintReceipt(PrintReceiptMapping.toSdk(printReceiptValue))
        Log.d(TAG, "Save print receipt configuration: $printReceiptValue")
